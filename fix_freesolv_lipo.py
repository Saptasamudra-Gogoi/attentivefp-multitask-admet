"""
Fix script for FreeSolv and Lipo — regression datasets
drop_last=True fixes BatchNorm crash on small batches
All 3 backbones: MoE-GIN, MoE-GCN, MoE-DMPNN
...
DATASETS  = [
    {"name": "FreeSolv", "metric": "rmse"},
    {"name": "Lipo",     "metric": "rmse"},
]
BACKBONES = ["MoE-GIN", "MoE-GCN", "MoE-DMPNN"]

print(f"Device : {DEVICE}")
print(f"Fixing: {[d['name'] for d in DATASETS]} x {BACKBONES}\n")
...
if os.path.exists(SAVE_PATH):
    ...
    for bb in list(results.keys()):
        for ds in ["FreeSolv", "Lipo"]:
            if ds in results.get(bb, {}):
                del results[bb][ds]
                print(f"Cleared {bb}/{ds} — will rerun cleanly")
...
def make_obj(trl, vl, bb, ic, ec):
    def obj(trial):
        ...
        return -best  # maximize negative RMSE
    return obj
...
for bb in BACKBONES:
    ...
    for ds_cfg in DATASETS:
        ...
        t0      = time.time()
        dataset = MoleculeNet(root=DATA_ROOT, name=ds)
        ...
        # drop_last=True prevents single-sample batch crashing BatchNorm
        ...
        study = optuna.create_study(
            study_name = f"{bb}_{ds}_{int(time.time())}",
            direction  = "maximize",  # maximizing -RMSE
        ...
        print(f"  Best val RMSE: {best_rmse:.4f} | {bp}")
        ...
        print(f"  ✓ [{bb}] {ds}: {mean_r:.4f} ± {std_r:.4f}  ({elapsed/60:.1f} min)")
...
print(f"\n{'='*55}")
print("DONE — Regression results:")
for bb in BACKBONES:
    for ds_cfg in DATASETS:
        ds = ds_cfg["name"]
        r = results.get(bb, {}).get(ds, {})
        if r:
            print(f"  {bb:<14} {ds:<10} RMSE: {r['mean']:.4f} ± {r['std']:.4f}")
