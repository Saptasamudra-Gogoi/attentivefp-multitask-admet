...
# ── TRUE D-MPNN: NNConv + GRUCell ─────────────────────────────────────────────

class DMPNN(nn.Module):
    """
    Genuine Chemprop-style directed MPNN.
    NNConv: edge-conditioned message passing where bond features
    parametrize the message transformation — each edge learns its
    own weight matrix as a function of its bond feature vector.
    GRUCell: recurrent node update, same as original Chemprop paper.
    """
    def __init__(self, in_ch, edge_ch, hidden, num_layers, dropout, num_tasks):
        ...
        for _ in range(num_layers):
            # NNConv: bond features → hidden×hidden weight matrix
    ...
    def forward(self, data):
        ...
        for conv, bn in zip(self.convs, self.bns):
            ...
            h = self.gru(m, h)          # GRU update: message + previous state
