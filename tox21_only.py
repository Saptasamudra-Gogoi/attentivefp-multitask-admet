"""
Tox21-only — AttentiveFP + Optuna
Same settings as main benchmark (30 trials, 100 epochs, patience 15, batch 64)
Results saved to results_from_tox21.json
"""

import json, time, warnings, shutil, os
import numpy as np
import torch
...
print(f"Device: {DEVICE}  |  Tox21 standalone run")

# ── Clear corrupted cache ───────────────────────────────────────────────────
tox21_processed = os.path.join(DATA_ROOT, "tox21", "processed")
if os.path.exists(tox21_processed):
    shutil.rmtree(tox21_processed)
    print("Cleared Tox21 processed cache — reprocessing...")
...
def run_training(train_loader, val_loader, test_loader, params, seed):
    ...
    np.random.seed(seed)
    model = AttentiveFP(
        in_channels=9, hidden_channels=params["hidden"], out_channels=12,
...
print(f"Tox21 dataset size: {len(dataset)}")
...
print(f"\n✓ Tox21: {mean_auc:.4f} ± {std_auc:.4f}  ({elapsed/60:.1f} min)")
