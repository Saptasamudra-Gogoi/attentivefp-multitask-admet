"""
TRUE D-MPNN — Lipo only (fast version)
Fix: smaller edge network (edge_ch → 32 → hidden) instead of (edge_ch → hidden*hidden)
This reduces NNConv memory and compute dramatically on large datasets.
"""
...
from torch.nn import Linear, BatchNorm1d, Sequential, GRUCell, ReLU
...
print("Backbone: TRUE D-MPNN (NNConv small edge net + GRUCell) — Lipo fast version")
...
class DMPNN(nn.Module):
    """
    True D-MPNN with SMALL edge network:
    edge_ch → 32 → hidden  (instead of edge_ch → hidden*hidden)
    Dramatically faster on large datasets, same architecture type.
    """
    def __init__(self, in_ch, edge_ch, hidden, num_layers, dropout):
        ...
        for _ in range(num_layers):
            # Small edge network: edge_ch → 32 → hidden (not hidden*hidden)
            nn_edge = Sequential(
                Linear(edge_ch, 32),
                ReLU(),
                Linear(32, hidden * hidden)
            )
...
def make_objective(train_data, val_data, in_dim, edge_dim):
    def objective(trial):
        ...
        num_layers = trial.suggest_int("num_layers",          2, 3)   # max 3 not 4
...
# Load existing — keep ESOL and FreeSolv, rerun Lipo only
if os.path.exists(SAVE_PATH):
    with open(SAVE_PATH) as f:
        results = json.load(f)
    # Keep ESOL and FreeSolv, force rerun Lipo
    results = {
        k: v for k, v in results.items()
        if k != "Lipo" and v.get("backbone") == "true_dmpnn_nnconv_grucell"
    }
    print(f"Kept {len(results)} existing results, rerunning Lipo\n")
else:
    results = {}
...
print(f"\n{'='*60}")
print(f"  TRUE D-MPNN (fast) | Lipo | RMSE")
print(f"{'='*60}")
...
study.optimize(
    ...
    n_trials=N_TRIALS,
    timeout=3600,    # 1 hour max for HPO
)
...
print(f"\n  ✔ TRUE D-MPNN Lipo: {mean_s:.4f} ± {std_s:.4f}  ({elapsed/60:.1f} min)")
print(f"  Saved → {SAVE_PATH}")
print(f"\n  All regression datasets complete:")
for k, v in results.items():
    print(f"    {k:12} RMSE: {v['mean']:.4f} ± {v['std']:.4f}")
